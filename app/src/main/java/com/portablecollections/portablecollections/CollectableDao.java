...
public interface CollectableDao {
     ...
     */
    @Query("SELECT id, name, description, country, city, imageUri, wantIt, gotIt FROM collectables")
    Cursor selectAll();
     ...
     */
    @Query("SELECT id, name, description, country, city, imageUri, wantIt, gotIt  FROM collectables WHERE id = :id")
